...
//     resume: null, // File object when selected
//     fullName: "",
//     email: "",
//     phone: "",
//     alternatePhone: "",
...
//   const [resumePreview, setResumePreview] = useState(null); // url or remote PDF
//   const [remoteResumeUrl, setRemoteResumeUrl] = useState(null); // if profile has resume url
...
//     // rawSkills may be:
//     // - ["aa, bb, cc"]  (single-string array from DB)
//     // - ["aa","bb","cc"] (correct array)
//     // - "aa,bb,cc" or null
//     if (!rawSkills) return [];
//     if (Array.isArray(rawSkills)) {
//       if (rawSkills.length === 1 && typeof rawSkills[0] === "string") {
//         return rawSkills[0]
//           .split(",")
//           .map((s) => s.trim())
//           .filter(Boolean);
//       }
//       // ensure every item trimmed if strings
//       return rawSkills
...
//         const p = res.profile;
//         const normalizedSkills = normalizeSkills(p.skills);
//         setFormData((prev) => ({
//           ...prev,
//           resume: null,
//           fullName: p.fullName || "",
...
//         // If API returns a resume URL, keep it for preview (adjust to your API key)
//         if (p.resume.path) {
//           setRemoteResumeUrl(p.resume.path);
//           setResumePreview(p.resume.path);
//         } else {
//           setRemoteResumeUrl(null);
...
//     setRemoteResumeUrl(null); // new local file takes precedence
...
//     if (!value) return;
//     // allow comma-separated multiple skills
//     const parts = value
//       .split(",")
//       .map((s) => s.trim())
//       .filter(Boolean);
...
//       // optional UX: delete last skill on backspace when input empty
...
//     try {
//       // validate using schema (ensure skills is array)
//       await schema.validate(formData, { abortEarly: false });

//       const formDataToSend = new FormData();
//       Object.entries(formData).forEach(([key, value]) => {
//         if (key === "skills" && Array.isArray(value)) {
//           // send as comma-separated string (backend expects this)
...
//             {/* If a local file is selected or remote URL exists, show name + preview */}
//             {(formData.resume || resumePreview) && (
...
//                       </p>
//                     )}
//                     {!formData.resume && remoteResumeUrl && (
//                       <p className="text-sm text-gray-500">
...
//                           setResumePreview(remoteResumeUrl || null);
//                         }}
//                       >
//                         Remove
//                       </button>
...
//                 {/* PDF Preview */}
//                 {resumePreview &&
//                 resumePreview.endsWith(".pdf") === false &&
//                 resumePreview.startsWith("http") === false &&
//                 formData.resume ? (
//                   // some browsers may not allow direct preview of some file names — but blob urls are fine
...
//                     {/* Use iframe for PDF preview (works for blob: and http(s) urls) */}
//                     <iframe
//                       src={resumePreview}
...
const schema = yup.object().shape({
  ...
  techStack: yup.string().required("Tech stack is required"),
  candidateSource: yup.string().required("Candidate source is required"),
  skills: yup.array().min(1, "Add at least 1 skill"), // set to 1 for flexibility
  description: yup.string().nullable(),
});
...
const EditProfile = () => {
  ...
  const [formData, setFormData] = useState({
    resume: null, // local File object (if user selects)
    fullName: "",
    email: "",
    phone: "",
    alternatePhone: "",
  ...
  // resumePreview: { url, name, type, isBlob } or null
  const [resumePreview, setResumePreview] = useState(null);
  // remoteResumeInfo: store info about resume from API (if any)
  const [remoteResumeInfo, setRemoteResumeInfo] = useState(null);
  // if user asks to remove the existing remote resume (on submit)
  ...
  const normalizeSkills = (raw) => {
    ...
    if (Array.isArray(raw)) {
      // if backend stored as single string inside array: ["a, b, c"]
  ...
  const handleRemoveSelectedFile = (e) => {
    ...
    // remove local selected file and revert to remote if exists
    setFormData((prev) => ({ ...prev, resume: null }));
    if (remoteResumeInfo) {
